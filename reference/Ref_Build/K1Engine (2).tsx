...
export const K1Engine: React.FC<K1EngineProps> = ({ compositorRect }) => {
  ...
  const params = useControls('K1 Lightwave Engine', {
    ...
    Optics: folder({
      ...
      prismOpacity: { value: K1_HERO_PRESET.optics.prismOpacity ?? 0.35, min: 0, max: 1, step: 0.01 },
    }),
    Physics: folder({
      motionMode: {
  ...
  const edgeLitUniforms = useMemo(
    () => ({
      ...
      uRailInner: { value: params.railInner },
      uRailOuter: { value: params.railOuter },
      uRailSigma: { value: params.railSigma },
    }),
  ...
  uniformsRef.current.uRailInner.value =
    effectiveOptics.railInner ?? K1_HERO_PRESET.optics.railInner;
  uniformsRef.current.uRailOuter.value =
    effectiveOptics.railOuter ?? K1_HERO_PRESET.optics.railOuter;
  uniformsRef.current.uRailSigma.value =
    effectiveOptics.railSigma ?? K1_HERO_PRESET.optics.railSigma;
